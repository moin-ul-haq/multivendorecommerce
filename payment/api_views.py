...
class PaymentApiViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def paypal(self, request, pk=None):
        ...
        if request.user.role != "admin" and payment.order.user != request.user:
            return Response({"detail": "forbidden"}, status=status.HTTP_403_FORBIDDEN)
        base_url = config(
            "PAYPAL_CHECKOUT_URL", default="https://www.paypal.com/checkoutnow"
        )
        query = urlencode(
            {
        ...
        return Response(
            {"checkout_url": f"{base_url}?{query}"}, status=status.HTTP_200_OK
        )
    ...
    def success(self, request, pk=None):
        ...
        send_order_status_email.delay(
            payment.order.user.email, payment.order.id, payment.order.status
        )
